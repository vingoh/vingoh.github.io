...
每个向量需要和三个内部权重不同但形状均为$$d_{model}*d_{model}$$的线性模型相乘，得到大名鼎鼎的QKV向量。文中这样解释：
...
这是一个相对简单的网络，如其名，每个token都要单独输入进这个网络，使用共享权重得到结果。通常结构是$$d_{model}-d_{ff}-d_{model}$$，且通常$$d_{ff} > d_{model}$$。
